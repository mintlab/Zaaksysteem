package Zaaksysteem::Model::Beheer::Import::GBA::BussumCsv;
use Moose;
...
package Zaaksysteem::Model::Beheer::Import::GBA::BussumCsv::_Iterator;
...
sub next {
    ...
    if ($self->data->[($self->_pointer - 1)]) {
        return $self->_parse_entry($self->data->[($self->_pointer - 1)]);
    }
...
sub _parse_entry {
    ...
    for (my $i = 0; $i < scalar(@{ $csv_map }); $i++) {
        ...
        $field = Unicode::String::latin1( $field );
