...
#        'naam'                      => $self->ldap_rs->get_value('cn') || undef,
#    });
#
#    $self->id($bo->id);
#
...
#        filter  => '(&(objectClass=organizationalUnit)(memberUid=' . $uid . '))',
...
#
#    $self->log->debug('HIBBEM? OK< THIS IS KILLING ME');
...
sub get_by_dn {
    ...
    my $ldapsearch = $ldap->search(
        filter  => '(&(objectClass=organizationalUnit)(ou=' . $org_ou . '))',
        base    => $dispatch_options->{customer}
            ->{start_config}
            ->{'LDAP'}
            ->{basedn},
