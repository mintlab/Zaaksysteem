...
sub verwijderen : Chained('base'): PathPart('verwijderen'): Args() {
    ...
    if (
        ...
        while (
            ...
            if (
                $zt_kenmerk->zaaktype_node_id->id eq
                $zt_kenmerk->zaaktype_node_id->zaaktype_id->zaaktype_node_id->id &&
                !$zt_kenmerk->zaaktype_node_id->zaaktype_id->deleted
            ) {
                $c->stash->{confirmation}->{message} =
                    'Helaas, dit kenmerk is in gebruik door een of meedere actieve zaaktypen.';
                $notused=0;
                next;
            }
            ...
            if ($zt_kenmerk->zaaktype_node_id->zaaks->count) {
                ...
                if ($zt_kenmerk->zaaktype_node_id->zaaks->search(
                        { status => { '!=' => 'deleted' }}
                    )->count
                ) {
                    $c->log->debug('Vond een actieve zaak met dit kenmerk');
                    $c->stash->{confirmation}->{message} =
