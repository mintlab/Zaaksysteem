package Zaaksysteem::Zaken::ComponentZaakKenmerken;
...
sub set_value {
    ...
    if ($self->multiple || $self->bibliotheek_kenmerken_id->type_multiple) {
        my @values;
...
sub _set_single_value {
    ...
    my $value   = $self->zaak_kenmerken_values->first;

    $self->result_source->schema->resultset('Logging')->add({
        zaak_id     => $self->zaak_id,
        component   => 'kenmerk',
        onderwerp   => substr('Kenmerk "' . $self->naam . '"'
            . ' gewijzigd naar: "' . (
                length($new) > 150
                    ? substr($new,0,150) . '...'
                    : $new
...
sub _set_multiple_value {
    ...
    $self->result_source->schema->resultset('Logging')->add({
        zaak_id     => $self->zaak_id,
        component   => 'kenmerk',
        onderwerp   => substr('Multiple kenmerk "' . $self->naam . '"'
